...
const Admin = () => {
  ...
  // Club Password Reset State
  const [passwordResetClubId, setPasswordResetClubId] = useState(null);
  const [newClubPassword, setNewClubPassword] = useState("");
  const [savingClubPassword, setSavingClubPassword] = useState(false);
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    if (!newClubPassword) return;
    setSavingClubPassword(true);
    try {
      await api.put(`/clubs/${passwordResetClubId}`, { password: newClubPassword });
      showToast("Club password updated successfully", "success");
      setPasswordResetClubId(null);
      setNewClubPassword("");
    } catch (err) {
      showToast(err.response?.data?.message || "Failed to update password", "error");
    } finally {
      setSavingClubPassword(false);
    }
  };
  ...
  const Badge = ({ children, type }) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded text-xs font-medium border ${style} capitalize`}
      >
        {children}
      </span>
    );
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-[#102a43] via-[#1e3a8a] to-[#2563eb] p-6 text-white shadow-xl sm:p-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">
            Keep QuEventHub moving.
          </h1>
          <p className="mt-2 max-w-xl text-sm leading-6 text-blue-100">
            Review users, manage clubs, and keep the campus calendar healthy.
          </p>
        </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <div>
              ...
              <button
                ...
                className="mt-2 text-sm text-indigo-600 hover:text-indigo-800 font-medium hover:underline"
              >
        ...
        <div className="flex items-end justify-between gap-4">
          ...
          <span className="hidden text-sm text-slate-500 sm:block">
            Search, review, and manage records
          </span>
        </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => {
                              setPasswordResetClubId(club._id);
                              setNewClubPassword(Math.random().toString(36).substring(2, 10));
                            }}
                            className="p-1.5 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded"
          ...
          <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <form onSubmit={handleUserSubmit} className="p-7 sm:p-9">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">
                    Q-ID (Roll No) <span className="text-red-500">*</span>
                  </label>
                  <input
                    required
                    className="w-full p-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                    value={userForm.q_id}
                    onChange={(e) =>
                      setUserForm({ ...userForm, q_id: e.target.value })
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUserForm({ ...userForm, course: e.target.value })
                    }
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={userForm.year}
                      onChange={(e) =>
                        setUserForm({ ...userForm, year: e.target.value })
                      }
                    />
                  </div>
                  <div>
              ...
              <div className="mt-8 flex flex-col-reverse justify-end gap-3 border-t border-slate-100 pt-6 sm:flex-row">
                <button
                  type="button"
                  onClick={() => setIsUserModalOpen(false)}
                  className="rounded-xl border border-slate-200 px-6 py-3 text-slate-600 transition hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={savingUser}
                  className="rounded-xl bg-indigo-600 px-7 py-3 font-bold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 disabled:opacity-50"
                >
                  {savingUser
                    ? "Saving..."
                    : editingUserId
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-7 py-5">
                ...
                <button onClick={() => setPasswordResetClubId(null)} className="rounded-xl p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700">
                    <X size={20} />
                </button>
            </div>
            ...
            <form onSubmit={handlePasswordSubmit} className="p-7">
                <div>
                    ...
                    <div className="flex gap-2">
                        <input required type="text" className="w-full p-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" 
                            value={newClubPassword} onChange={e => setNewClubPassword(e.target.value)} placeholder="Enter new password" />
                        <button type="button" onClick={() => setNewClubPassword(Math.random().toString(36).substring(2, 10))} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
                            Generate
                        </button>
                    </div>
                </div>
                ...
                <div className="mt-8 flex justify-end gap-3 border-t border-slate-100 pt-6">
                  <button type="button" onClick={() => setPasswordResetClubId(null)} className="rounded-xl border border-slate-200 px-6 py-2 text-slate-600 transition hover:bg-slate-50">Cancel</button>
                  <button type="submit" disabled={savingClubPassword} className="rounded-xl bg-indigo-600 px-6 py-2 font-bold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 disabled:opacity-50">
                        {savingClubPassword ? "Saving..." : "Update Password"}
                    </button>
                </div>
            </form>
          </div>
